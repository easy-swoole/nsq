 ...
 * Time: 9:53 AM
 */
namespace EasySwoole\Nsq\Wire;

use EasySwoole\Nsq\Exception\ReadException;
...
class Unpack
{
    ...
    public static function getFrame($buffer)
    {
        ...
        switch ($frame['type']) {
            ...
            default:
                throw new ReadException(substr($buffer, 8));
        }
     ...
     * @return bool
     */
    private static function checkError(array $frame, $type)
    {
        return isset($frame['type'], $frame['error'])
            && $frame['type'] === $type;
    }
    ...
    {
        return self::checkError($frame, self::FRAME_TYPE_ERROR);
    }
